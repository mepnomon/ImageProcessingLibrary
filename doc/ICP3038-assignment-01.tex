\def\DEADLINE{Friday 11th November 2016 (midnight)}
...
\item Assignment operator, also called copy operator:
    \begin{itemize}
	\item \verb+Image& operator=(const Image& anImage)+; (assignment operator, also called copy operator)
	\end{itemize}

\item Read/Write an image from/to an ASCII file:
    \begin{itemize}
	\item \verb+void readASCII(const std::string& aFileName)+
	\item \verb+void writeASCII(const std::string& aFileName) const+
	\end{itemize}

\item Arithmetic operators using an another Image as a parameter:
    \begin{itemize}
...
\item Arithmetic assignment operators using an another Image as a parameter:
    \begin{itemize}
...
\item Arithmetic operators using a floating point number as a parameter:
    \begin{itemize}
...
\item Arithmetic assignment operators using a floating point number as a parameter:
    \begin{itemize}	
...
\item Negation operator:
    \begin{itemize}	
	\item \verb+Image operator!()+; (return the negative image)
	\end{itemize}

\item Accessors:
    \begin{itemize}	
	\item \verb+unsigned int getWidth() const+ (accessor on the width of the image)
	\item \verb+unsigned int getHeight() const+ (accessor on the height of the image)
	\item \verb+double getAspectRatio() const+ (method returning the aspect ratio)
	\end{itemize}

\item Setters and Getters
    \begin{itemize}	
	\item \verb+void setPixel(unsigned int i, unsigned int j, float v)+ (method to set the value (v) of a pixel given its horizontal (i) and vertical (j) indices)
	\item \verb+float getPixel(unsigned int i, unsigned int j) const+ (method returning the value of a pixel given its horizontal and vertical indices)
    \item A method returning the smallest value in the image
    \item A method returning the largest value in the image
	\end{itemize}

	
%\item A method normalising the image between 0 and 1; (you can call the \verb+shiftScaleFilter method+)
...
\subsection*{Task 3: Write a short logbook}

Your final submissions should include the code and a logbook to provide evidence of testing. 
...
\noindent For testing, you can use ImageJ to load and save ASCII files (see \url{http://imagej.nih.gov/ij/download.html}). 
...
Please remember that marks are provisional until they are confirmed by a board of examiners. \\[2em]
...
\noindent The marking will take into account:
\begin{itemize}
\item What is implemented, what is not implemented;
\item The amount and usefulness of comments;\item The naming of functions, variables, etc. (including the consistency in the coding standard);\item Remove old code;\item Testing;\item Verbal explanation in the lab after the submission. \\[2em]
\end{itemize}
